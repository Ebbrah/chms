...
import { updateJumuiya } from "@/lib/actions/admin-setup";
...
export function EditJumuiyaForm({
  id,
  name,
  chairpersonUserId,
  chairpersonOptions,
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    fd.set("id", id);
    const res = await updateJumuiya(fd);
    if ("error" in res && res.error) {
      setMsg(res.error);
      return;
    }
    setMsg("Jumuiya updated.");
    router.push("/dashboard/settings/jumuiya");
    router.refresh();
  }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">Edit jumuiya</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={(e) => void onSubmit(e)} className="grid gap-4 sm:max-w-xl">
          ...
          <div className="grid gap-2">
            ...
            <Input id="jumuiya-name" name="name" defaultValue={name} required />
          </div>
          <div className="grid gap-2">
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" variant="outline" onClick={() => router.push("/dashboard/settings/jumuiya")}>
              Cancel
            </Button>
          </div>
        </form>
